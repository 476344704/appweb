<template>
  <pull-to
    ...
    <header class="header-box" >
      ...
      <div class="title">Facebook Cover</div>
    </header>
    <div class="container">
      <div class="waterfall1">
        <div v-for="temp in templates" :key="temp.index" class="item1">
          <a v-on:click="getdata(temp)"><img :src="temp.originalsrc" :alt="temp.pname" /></a>
        </div>
      </div>
    </div>
  </pull-to>
...
export default {
  ...
  methods: {
    getTemps () {
      this.getPage = 1
      this.$api.get('poster/iosFbCovers.ashx?page=' + this.getPage, null, r => {
        let tempsvar = r.data
        let templist = []
